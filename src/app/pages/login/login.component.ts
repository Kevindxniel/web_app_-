  ...
  login(){
    ...
    if(this.showRegister){
      ...
      this.authService.registerWithEmail(this.email?.value, this.password?.value)
        .then(() => {
          ...
          this.router.navigate(['/products']);  // Redirigir a /products después de registrarse
        })
        .catch(err => {
          console.log(err);
    ...
    this.authService.loginWithEmail(this.email?.value, this.password?.value)
      .then(() => {
        this.errorMessage = "";
        this.usersService.loadUserInFirebase();
        this.router.navigate(['/products']);  // Redirigir a /products después de iniciar sesión
      })
      .catch(err => {
        console.log(err);
        this.errorMessage = err.message;
  ...
  registerWithGoogle(){
    this.authService.loginWithGoogle()
      .then(() => {
        this.errorMessage = "";
        this.usersService.loadUserInFirebase();
        this.router.navigate(['/products']);  // Redirigir a /products después de iniciar sesión con Google
      })
      .catch(err => {
        console.log(err);
        this.errorMessage = err.message;
